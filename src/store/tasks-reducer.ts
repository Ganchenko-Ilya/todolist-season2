...
export const tasksReducer = (state: TaskObjType = initialTasksState, action: ActionsTasksType): TaskObjType => {
  switch (action.type) {
    case "ADD-TASK": {
      return {
        ...state,
...
export const addTaskTC =
  ...
  async (dispatch) => {
    ...
    try {
      ...
      helpResultFunc(() => dispatch(addTaskAC(tId, res.data.data.item)), res, dispatch, "Add Task!");
    } catch (error) {
      errorCatchHelpFunc(dispatch, error);
    }
  };
...
export const changeTaskTC =
  ...
  async (dispatch, getState) => {
    ...
    try {
      ...
      helpResultFunc(() => dispatch(changeTaskAC(tId, id, itemChangedModel)), res, dispatch, "Task changed!");
    } catch (error) {
      errorCatchHelpFunc(dispatch, error);
    }
  };
